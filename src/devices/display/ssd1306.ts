...
import { SSD1306Element } from '@wokwi/elements';
import { I2CDevice } from "../arduino/internal/i2c-bus";
...
export class SSD1306Controller implements I2CDevice, Component {
  ...
  init(arduino:ArduinoUno, runner: AVRRunner) {
  ...
  update(arduino:ArduinoUno) {
    let result = null;

    if (this.scrollActive && arduino.cpuMillis() > this.nextScrollTime) {
      this.scroll();
    ...
    // Update SSD1306
    this.toImageData(this.ui.imageData);
    this.ui.redraw();
  ...
  calculateNextScroll() {
    this.nextScrollTime = this.arduino.cpuMillis() + this.scrollFrameTime;
  }
